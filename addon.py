...
if KodiV >= 19:
    ...
    bnt1 = xbmcvfs.translatePath(os.path.join(__cwd__, 'resources',
                                 'bnt1.png'))
    bnt2 = xbmcvfs.translatePath(os.path.join(__cwd__, 'resources',
                                 'bnt2.png'))
    bnt3 = xbmcvfs.translatePath(os.path.join(__cwd__, 'resources',
                                 'bnt3.png'))
    bnt4 = xbmcvfs.translatePath(os.path.join(__cwd__, 'resources',
                                 'bnt4.png'))
    bnt = xbmcvfs.translatePath(os.path.join(__cwd__, 'icon.png'))
else:
    __cwd__ = xbmc.translatePath(__addon__.getAddonInfo('path'
                                 )).decode('utf-8')
    bnt1 = xbmc.translatePath(os.path.join(__cwd__, 'resources',
                              'bnt1.png')).decode('utf-8')
    bnt2 = xbmc.translatePath(os.path.join(__cwd__, 'resources',
                              'bnt2.png')).decode('utf-8')
    bnt3 = xbmc.translatePath(os.path.join(__cwd__, 'resources',
                              'bnt3.png')).decode('utf-8')
    bnt4 = xbmc.translatePath(os.path.join(__cwd__, 'resources',
                              'bnt4.png')).decode('utf-8')
    bnt = xbmcvfs.translatePath(os.path.join(__cwd__, 'icon.png'
                                )).decode('utf-8')
...
items = [{
    ...
    'title': "БНТ 1",
    'url': 'https://bnt.bg/bnt1/shows',
    'action': 'list_shows',
    'thumb': bnt1,
    }, {
    'title': "БНТ 2",
    'url': 'https://bnt.bg/bnt2/shows',
    'action': 'list_shows',
    'thumb': bnt2,
    }, {
    'title': '\xd0\x91\xd0\x9d\xd0\xa2 3',
    'url': 'https://bnt.bg/bnt3/shows',
    'action': 'list_shows',
    'thumb': bnt3,
    }, {
    'title': '\xd0\x91\xd0\x9d\xd0\xa2 4',
    'url': 'https://bnt.bg/bnt4/shows',
    'action': 'list_shows',
    'thumb': bnt4,
    }]
items_live = [{
    ...
    'title': "БНТ 2",
    'url': 'http://tv.bnt.bg/bnt2',
    'action': 'live',
    'thumb': bnt2,
    }, {
    'title': '\xd0\x91\xd0\x9d\xd0\xa2 3',
    'url': 'http://tv.bnt.bg/bnt3',
    'action': 'live',
    'thumb': bnt3,
    }, {
    'title': '\xd0\x91\xd0\x9d\xd0\xa2 4',
    'url': 'http://tv.bnt.bg/bnt4',
    'action': 'live',
    'thumb': bnt4,
    }]
...
def get_shows(url):
    ...
    for i in range(0, len(links)):
        title = links[i]['title']
        imgs = links[i].find('img')
        item = {'title': title, 'url': links[i]['href'],
                'thumb': imgs['src']}
        shows.append(item)
...
def get_parts(url):
    ...
    for i in range(0, len(links)):
        if img_holder[i].find_all('div', {'class': 'whole-show'}):
            title = '[COLOR red]' + "Цялото предаване - " + '[/COLOR]' \
                + titles_ful_episode[0].get_text()
        else:
...
def list_live():
    ...
    for item in items_live:
        ...
        list_item.setArt({'icon': item['thumb'], 'thumb': item['thumb'
                         ], 'fanart': item['thumb']})
...
def list_shows(shows):
    ...
    for show in shows:
        ...
        list_item.setArt({'icon': show['thumb'], 'thumb': show['thumb'
                         ], 'fanart': show['thumb']})
...
def list_episodes(episodes):
    ...
    for episode in episodes:
        ...
        if episode['title'] != next_page_title and episode.get('parts') \
            is False:
            url = make_url('play', episode['url'])
            list_item.setProperty('IsPlayable', 'true')
            list_item.setInfo('video', {'title': episode['title']})
            is_folder = False
        elif episode['title'] != next_page_title and episode.get('parts'
                ) is True:
        ...
        list_item.setArt({'icon': episode.get('thumb'),
                         'thumb': episode.get('thumb'),
                         'fanart': episode.get('thumb')})
        listing.append((url, list_item, is_folder))
...
def list_parts(parts):
    ...
    for part in parts:
        ...
        list_item.setArt({'icon': part['thumb'], 'thumb': part['thumb'
                         ], 'fanart': part['thumb']})
...
def router(param_str):
    ...
    if params:
        ...
        for item in items:
            is_dir = True
            list_item = xbmcgui.ListItem(label=item['title'])
            list_item.setArt({'icon': item['thumb'],
                             'thumb': item['thumb'],
                             'fanart': item['thumb']})
            url = make_url(item['action'], item['url'])
            listitems.append((url, list_item, is_dir))
